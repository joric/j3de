...
enum clax_3ds_chunks_ { /* Chunk ID */
  ...
  NAMED_OBJECT       = 0x4000, CHUNK_TRIMESH      = 0x4100,
  ...
  CHUNK_OBJECT_NODE_TAG = 0xB002, NODE_HDR = 0xB010,
  ...
  CHUNK_NODE_ID		 = 0xB030, CHUNK_TRACKCAMERA  = 0xB003,
  ...
  CHUNK_INSTANCE_NAME    = 0xB011, CHUNK_MAPROTANGLE  = 0xA35C,
...
static int read_NODE_ID    (FILE *f); /* Object number           */
static int read_NODE_HDR (FILE *f); /* Track object name       */
static int read_INSTANCE_NAME    (FILE *f); /* Dummy object name       */
...
static c_LISTKEY key_chunks[] = { /* Keyframer chunks */
  ...
  {CHUNK_OBJECT_NODE_TAG,    1, read_NULL},
  {CHUNK_FRAMES,       0, read_FRAMES},
  {NODE_HDR,		   0, read_NODE_HDR},
  {CHUNK_INSTANCE_NAME,    0, read_INSTANCE_NAME},
...
*/
  ...
  while (nv-- > 0) {
    ...
    fc->b = c[1];
    fc->c = c[2];
    fc->p[0] = &obj->vertices[c[0]];
    fc->p[1] = &obj->vertices[c[1]];
    fc->p[2] = &obj->vertices[c[2]];
    fc->flags = 0;
    fc->mat = 0;
...
static int read_NODE_ID (FILE *f)
{
/*
  read_NODE_ID: Object number reader. (3DS 4.0+)
*/
  word n;
...
static int read_INSTANCE_NAME (FILE *f)
{
/*
  read_INSTANCE_NAME: Dummy object name reader.
*/
...
static int read_NODE_HDR (FILE *f)
{
/*
  read_NODE_HDR: Track object name reader.
*/
  ...
  if (c_chunk_prev != CHUNK_NODE_ID) c_id++;
  ...
  if (c_chunk_last == CHUNK_OBJECT_NODE_TAG) {
    obj->parent = wparent;
